...
rem dir /s /b %bin_to_load% ^| findstr "BUILD" ^| findstr "ARDUINO_NANO33BLE_SENSE" ^| findstr "%bin_to_load%"
rem ^| findstr "ARDUINO_NANO33BLESENSE" ^| findstr /v /i "mbed-os" ^| findstr "%2"'
for /f "delims=" %%i in ('dir /s /b %bin_to_load% ^| findstr "BUILD" ^| findstr "ARDUINO_NANO33BLESENSE"') do (
	rem echo "%%i"
	set bin_to_load=%%i
)
